...
enum charger_sdp {
	SDP_INPUT_CURRENT = 0x08,
	SDP_INPUT_VOLTAGE = 0x18,  //5v
	SDP_CHARGE_CURRENT = 0x08,
	SDP_CHARGE_VOLTAGE420 = 0x5e,
	SDP_CHARGE_VOLTAGE435 = 0x82,
	SDP_CHARGE_VOLTAGE440 = 0x8e,
};

enum charger_cdp {
	CDP_INPUT_CURRENT = 0x1c,
	CDP_INPUT_VOLTAGE = 0x18,  //5v
	CDP_CHARGE_CURRENT = 0x18,
	CDP_CHARGE_VOLTAGE420 = 0x5e,
	CDP_CHARGE_VOLTAGE435 = 0x82,
	CDP_CHARGE_VOLTAGE440 = 0x8e,
};

enum charger_dcp {
	DCP_INPUT_CURRENT = 0x30,
	DCP_INPUT_VOLTAGE = 0x18,  //5v
	DCP_CHARGE_CURRENT = 0x28,
	DCP_CHARGE_VOLTAGE420 = 0x5e,
	DCP_CHARGE_VOLTAGE435 = 0x82,
	DCP_CHARGE_VOLTAGE440 = 0x8e,
};
...
struct bq25890_device {
	...
	struct delayed_work normal_work;
				  ...
				  struct bq25890_state *state)
{
	...
	if(state->chrg_status == 0) {
		bq25890_field_write(bq, F_IILIM, SDP_INPUT_CURRENT);
		bq25890_field_write(bq, F_VINDPM_OFS, SDP_INPUT_VOLTAGE);
		bq25890_field_write(bq, F_ICHG, SDP_CHARGE_CURRENT);
		// dev_info(bq->dev, "INT_INPUT_CURRENT:%x, INT_INPUT_VOLTAGE:%x, INT_CHARGE_CURRENT:%x\n",
...
static int bq25890_charger_evt_notifier(struct notifier_block *nb,
					unsigned long event,
					void *ptr)
{
	struct bq25890_device *bq =
		container_of(nb, struct bq25890_device, cable_cg_nb);

	queue_delayed_work(bq->usb_charger_wq, &bq->normal_work,
	msecs_to_jiffies(10));
...
static void bq25890_charger_evt_handle(struct bq25890_device *bq, struct extcon_dev *edev)
{
	struct bq25890_state state;
	...
	if (extcon_get_state(edev, EXTCON_CHG_USB_SDP) > 0) {
		...
		dev_info(bq->dev,"xcp:USB_TYPE_USB_CHARGER\n");

		bq25890_field_write(bq, F_IILIM, SDP_INPUT_CURRENT);
		bq25890_field_write(bq, F_VINDPM_OFS, SDP_INPUT_VOLTAGE);
		bq25890_field_write(bq, F_ICHG, SDP_CHARGE_CURRENT);
		dev_info(bq->dev, "INPUT_CURRENT:%x, INPUT_VOLTAGE:%x, CHARGE_CURRENT:%x\n",
		SDP_CHARGE_CURRENT, SDP_INPUT_VOLTAGE, SDP_CHARGE_CURRENT);

	} else if (extcon_get_state(edev, EXTCON_CHG_USB_DCP) > 0) {
		charger_state = USB_TYPE_AC_CHARGER;
		dev_info(bq->dev,"xcp:USB_TYPE_AC_CHARGER\n");

		bq25890_field_write(bq, F_IILIM, DCP_INPUT_CURRENT);
		bq25890_field_write(bq, F_VINDPM_OFS, DCP_INPUT_VOLTAGE);
		bq25890_field_write(bq, F_ICHG, DCP_CHARGE_CURRENT);
		dev_info(bq->dev, "INPUT_CURRENT:%x, INPUT_VOLTAGE:%x, CHARGE_CURRENT:%x\n",
		DCP_INPUT_CURRENT, DCP_INPUT_VOLTAGE, DCP_CHARGE_CURRENT);

	} else if (extcon_get_state(edev, EXTCON_CHG_USB_CDP) > 0) {
		charger_state = USB_TYPE_CDP_CHARGER;
		dev_info(bq->dev,"xcp:USB_TYPE_CDP_CHARGER\n");

		bq25890_field_write(bq, F_IILIM, CDP_INPUT_CURRENT);
		bq25890_field_write(bq, F_VINDPM_OFS, CDP_INPUT_VOLTAGE);
		bq25890_field_write(bq, F_ICHG, CDP_CHARGE_CURRENT);
		dev_info(bq->dev, "INPUT_CURRENT:%x, INPUT_VOLTAGE:%x, CHARGE_CURRENT:%x\n",
		CDP_INPUT_CURRENT, CDP_INPUT_VOLTAGE, CDP_CHARGE_CURRENT);
	}

	bq25890_get_chip_state(bq, &state);
	bq->state = state;
	power_supply_changed(bq->charger);
}

static void bq25890_charger_evt_worker(struct work_struct *work)
{
	struct bq25890_device *bq = container_of(work,
		struct bq25890_device, normal_work.work);
	struct extcon_dev *edev = bq->cable_edev;

	bq25890_charger_evt_handle(bq, edev);
}

static int bq25890_register_cg_extcon(struct bq25890_device *bq,
				...
				struct notifier_block *able_cg_nb)
{
	...
	ret = extcon_register_notifier(edev,
				EXTCON_CHG_USB_SDP,
					able_cg_nb);
	if (ret < 0) {
		dev_err(bq->dev, "failed to register notifier for SDP\n");
		return -1;
	}
	...
	ret = extcon_register_notifier(edev,
				EXTCON_CHG_USB_DCP,
					able_cg_nb);
	if (ret < 0) {
		dev_err(bq->dev, "failed to register notifier for DCP\n");
		return -1;
	}
	...
	ret = extcon_register_notifier(edev,
				EXTCON_CHG_USB_CDP,
					able_cg_nb);
	if (ret < 0) {
		dev_err(bq->dev, "failed to register notifier for CDP\n");
		return -1;
	}
...
static int bq25890_register_cg_psy(struct bq25890_device *bq)
{

	if (bq->cable_edev) {
		/* Register chargers  */
		printk("xcp_%s,%d\n", __func__, __LINE__);
		INIT_DELAYED_WORK(&bq->normal_work,
		bq25890_charger_evt_worker);
		bq->cable_cg_nb.notifier_call =
			bq25890_charger_evt_notifier;
		bq25890_register_cg_extcon(bq, bq->cable_edev,
					    &bq->cable_cg_nb);
	}
...
static int bq25890_register_pd_psy(struct bq25890_device *bq)
{
	...
	if (bq->nb.notifier_call) {
		...
		if (IS_ERR_OR_NULL(notify_psy)) {
			dev_info(bq->dev, "bq25890 notify_psy is error\n");
			notify_psy = NULL;
		}
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	bq->usb_charger_wq = alloc_ordered_workqueue("%s",
				WQ_MEM_RECLAIM |
				WQ_FREEZABLE,
				"bq25890-pd-wq");
